    ...
    product.value.options[index].optionValues = [
        {
            code: uuidv4(),
            value:
                product.value.options[index].visual == 1
                    ? defaultColor.value
                    : "",
            label: "",
            image: "",
...
const toggleAddOptionValue = (optionIndex: any) => {
    ...
    const value = {
        ...
        code: uuidv4(),
        value:
            product.value.options[optionIndex].visual == 1
                ? defaultColor.value
                : "",
        label: "",
        image: "",
    ...
    product.value.skus = generateCombinations(arrCodeOptionValue).map(
        (item: any) => {
            if (typeof item == "string") {
                ...
                item = item.map((i: any) =>
                    mapOptionValue(arrOptionValueFlat, i)
                );
            }
...
watch(
    ...
    (newVal, oldVal) => {
        product.value.numberWarning = isNaN(Number(newVal))
            ? 0
            : Number(newVal);
    }
);
