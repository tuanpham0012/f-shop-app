                            ...
                            <div class="col-span-16 md:col-span-20 grid grid-cols-16 md:grid-cols-20 gap-1 md:gap-4 ps-1">
                                <div class="col-span-16 md:col-span-10 flex items-center">
                                    <div class="grid grid-cols-10 gap-2 pt-[4px] w-100">
                                        ...
                                        <div class="col-span-16 md:col-span-4 text-xs md:text-sm text-gray-400 md:inline-block" v-if="item.sku.variants.length > 0">
                                            <p class="grid grid-cols-3 gap-3 w-full text-xs leading-none text-gray-800 mb-2" v-for="(variant, index) in item.sku.variants" :key="index">
                                                <span class="text-gray-400">{{ variant.optionName }}: </span> <span class="col-span-2">{{ variant.valueName }}</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-span-8 md:col-span-3 flex items-center justify-start md:justify-end py-2 text-sm">{{ displayPrice(item.sku.price) + " đ" }}</div>
